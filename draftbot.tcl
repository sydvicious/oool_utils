...
proc add_player_to_roster {owner sydID} {
  global replacement_level
  global debug
...
# to be compared at other positions.
  ...
  if {$eligible_SP == 1} {
	...
	if {$points > $pitcher_points} {
	  ...
	  if {[add_player_to_roster $owner $pitcher] eq ""} {
	...
	foreach newpos "OF3 OF2 OF1" {
	  ...
	  if {$existing_player eq ""} {
	    ...
	    if {$points > $existing_points} {
	      ...
	      place_player_in_roster $owner $sydID $newpos
	      if {[add_player_to_roster $owner $existing_player] eq ""} {
...
while {[gets $fd line] >= 0} {
  ...
  add_player_to_roster $owner $sydID
...
proc do_pick {pick_owner round} {
  ...
  if {$player eq ""} {
    ...
    for {set i 0} {$i < $length} {incr i} {
      ...
      debug_pause
      set current_owner [get_property $candidate "owner"]
      if {$current_owner eq ""} {
...
set num_players [get_property "Syd" "num_players"]
set round 1
set skipped_players {}
while {$num_players < 35} {
  set player [do_pick "Syd" $round]
  if {$player eq ""} {
    puts "Ran out of players."
    break
  } else {
    set found 0
    while {!$found} {
      puts "$player - [get_property $player "name"]?"
      puts {Type "t" for Taken (or don't ever Take); "n" for Not now; "y" for Yes}
      set line [gets stdin]
      set found 1
      switch -exact -- $line {
	"t" {
	  debug_puts "Taken."
	  debug_pause
	  set_property $player "owner" "taken"
	}
	"n" {
	  debug_puts "Skipped."
	  debug_pause
	  lappend skipped_players $player
	  set_property $player "owner" "skipped"
	}
	"y" {
	  debug_puts "Yes."
	  debug_pause
	  if {[add_player_to_roster "Syd" $player] eq ""} {
	    debug_puts "No place for [get_property $player "name"]"
	    debug_pause
	    lappend skipped_players $player
	    set_property $player "owner" "skipped"
	  } else {
	    debug_puts "Added [get_property $player "name"]"
	    set_property $player "owner" "Syd"
	    debug_pause
	  }
	}
	"default" {
	  puts "Please choose a valid response. $line is not."
	  set found 0
	}
      }
    }
    foreach skipped $skipped_players {
      set_property $skipped "owner" {}
      debug_pause
    }
    set skipped_players {}
  }
  set num_players [get_property "Syd" "num_players"]
  debug_puts "num_players - $num_players"
}
